...
- [ ] Contact form built 2026-09-09 (honeypot + reCAPTCHA v2 checkbox, code in core/forms.py, core/recaptcha.py, core/views.py), replacing the mailto link Eddy flagged as bot bait. Mail goes out through the Workspace SMTP relay service (Gmail routing settings) that was already set up for EdNoKa: 74.208.9.220 (this server, listed there as "GodotHire server") is already an allowed IP, so no SMTP password needed at all, EMAIL_HOST is smtp-relay.gmail.com with no auth. Progress 2026-09-09: Eddy registered playworksnorth.com at google.com/recaptcha/admin and gave the site key + secret key. Left: Eddy appends RECAPTCHA_SITE_KEY/RECAPTCHA_SECRET_KEY to /root/playworksnorth.com/.env and restarts the service (I have no SSH access to the server); the code deploys on the next push. Until then the form runs on Google's public test keys and shows a friendly error instead of sending
