...
- [x] 2026-09-09 Zenith Caller card art off the studio teal and onto the project's own colour (Eddy: every card uses its own project's palette, not the site's). It uses the chevron alone, which is the settled half of that mark, since the container is still open in ZenithCaller's own TODO (review: icon-direction). Hot signal red-orange #FF5A36 on near-black, so it reads as Apex rather than as this website. **Provisional**: redo it from the real mark once Eddy picks the container, and if he wants a different brand colour than the one chosen here, this is the file to change
