...
- [ ] Contact form built 2026-09-09 (honeypot + reCAPTCHA v2 checkbox, code in core/forms.py, core/recaptcha.py, core/views.py), replacing the mailto link Eddy flagged as bot bait. Two things only Eddy can do are left before it actually sends mail: (1) sign in at google.com/recaptcha/admin with the studio's Google account, register playworksnorth.com (and www.), pick v2 checkbox, hand over the site key and secret key; (2) get SMTP credentials able to send as contact@playworksnorth.com (a Workspace app password, needs 2-Step Verification on, or the Workspace SMTP relay service allowlisting 74.208.9.220) and hand over the address/password. Both pairs go as RECAPTCHA_SITE_KEY/RECAPTCHA_SECRET_KEY and EMAIL_HOST_USER/EMAIL_HOST_PASSWORD in /root/playworksnorth.com/.env on the server (systemd reads it, nothing to commit). Until then the form runs on Google's public test keys and shows a friendly error instead of sending
